...
class Tabel_form(forms.ModelForm):
    ...
    def clean(self):
        super().clean()
        exist_tabels = Tabel.objects.filter(year=self.cleaned_data['year']).filter(type_id=1).filter(month=self.cleaned_data['month']).filter(department=self.cleaned_data['department']).filter(del_check=0)
