        ...
        if dep !=0 and per != 0:
            ...
            if dep != 0:
                ...
                if per != 0:
                    ...
                    if terr == 1:
                            items_main = VacantionSheduleItem.objects.filter(bound_shed__year=year).filter(dur_from__month=per).exclude(move_from__isnull=False).filter(~Q(emp__department__name__contains='Елизово') | ~Q(emp__aup__name__contains='Елизово') ).values('id', 'emp__department__name' , 'emp__aup__name' , 'emp', 'emp__fullname', 'dur_from', 'dur_to', 'days_count', 'move_from', 'move_to', 'child_year', 'days_count_move', 'city', 'emp__position__name')
                            items_move = VacantionSheduleItem.objects.filter(bound_shed__year=year).filter(~Q(emp__department__name__contains='Елизово') | ~Q(emp__aup__name__contains='Елизово') ).filter(move_from__month=per).values('id', 'emp__department__name' , 'emp__aup__name' , 'emp', 'emp__fullname', 'dur_from', 'dur_to', 'days_count', 'move_from', 'move_to', 'child_year', 'days_count_move', 'city', 'emp__position__name')
                            items = items_main.union(items_move).order_by('emp__department__union__name', 'emp__aup__name', 'emp__department__name',  'emp', 'id', 'dur_from')
                    else:
                        if terr == 2:
